...
const listData = computed(()=> store.state.chatData.users);
// const contactList = computed(()=> store.state.chatData.contacts);
...
const username = ref("Chat");
let receiver_status = ref(false);
const chatForm = ref(false);
...
const openChat = (contact)=> {
  if(Object.keys(contact).length > 0){
    ...
    // const data = 
    // openUser.value = computed(()=> store.state.chatData.users.find(item => item.id == contact.id));
    // if (openUser.value.value.chat.messages && openUser.value.value.chat.messages.length > 0) { 
    //   groupedMessages.value = computed(() => {
    //       const grouped = {};
    //       openUser.value.value.chat.messages.forEach((message) => {
    ...
    // receiver_status.value = contact.online_status;
    // store.commit("setCurrentReceiver", contact);
  }
...
const messageForm = () => {
  ...
  // // if (openUser.value && openUser.value.chat) {
  //   openUser.value.chat.messages.push(data);
  // // }
...
watchEffect(()=> {
  // openUser.value = computed(()=> store.state.chatData.users[0])
  // if(openUser.value.length){
  //   groupedTheMessage()
  // }
  ...
  groupedContacts.value = computed(()=> {
    ...
    // Sort users by name
    const sortedUsers = listData.value.sort((a, b) =>
  ...
  currentUser.value = computed(()=> listData.value.find(item => item.id == currentID.value));
  
  // groupedMessages.value = computed(() => {
  //         const grouped = {};
  //         currentUser.value.chat.messages.forEach((message) => {
  //           const day = message.label; // Assuming the label field indicates the day
  //           if (!grouped[day]) {
  //             grouped[day] = [];
  //           }
...
<template>
  <Layout>
    ...
    {{currentUser}}
    <div class="d-lg-flex">
      <div class="chat-leftsidebar me-lg-4 bg-white px-3">
        <div class="">  
                  ...
                  <div>
                    ...
                    <simplebar style="max-height: 410px" id="my-element">
                      <ul class="list-unstyled chat-list">
                       <!--  <li v-for="data of listData.data.recents" :key="data.id" @click="openChat(data)"
