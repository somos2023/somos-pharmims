      ...
      if(userData.role_id == 3){
        return (
          (item.barcode.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
           ...
           item.dosage_form.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
           item.unit_of_measure.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
           item.price.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
           item.stock.toLowerCase().includes(searchQuery.value.toLowerCase()))
        );
      } else {
        return (
          (item.barcode.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
           ...
           item.dosage_form.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
           item.unit_of_measure.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
           item.selling_price.includes(searchQuery.value) ||
           item.stock.includes(searchQuery.value)
           )
        );
      }
    });
  ...
  <Layout>
    ...
    <div v-else> 
      <div v-if="listData.length">
        ...
        <TableComponent :roleID="userData.role_id" :items='servicesPaginated' :columns="columnData" model="product" :pages="displayedPages" :current="currentPage" :human="currentPageHuman" :num_pages="numPages" @change="changePage" @delete="deleteItem" :hideActions="userData.role_id == 1" />
      </div>
      <div v-else>
        <div class="row">
          <div class="col-md-12">
